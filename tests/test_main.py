...
class TestPresenceMonitoringApp:
    ...
    @patch('time.time')
    def test_run_loop(self, mock_time, mock_get_logger, mock_sleep, mock_tracker, mock_db, mock_scanner):
    ...
    @patch('time.sleep')
    @patch('time.time')
    def test_error_handling(self, mock_time, mock_sleep, mock_tracker, mock_db, mock_scanner):
