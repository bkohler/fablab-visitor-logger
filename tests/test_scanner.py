...
class TestBLEScanner(unittest.TestCase):
    ...
    def test_scan_with_valid_data(self, mock_scanner):
        ...
        mock_device.scanData = {
            ...
            0x19: 0,  # APPEARANCE
            0x16: {"0000180a-0000-1000-8000-00805f9b34fb": b'test'}  # SERVICE_DATA
        }
        ...
        self.assertEqual(devices[0]["service_uuids"], ["0000180a-0000-1000-8000-00805f9b34fb"])
        self.assertEqual(devices[0]["manufacturer_data"], {"65535": "74657374"})
        self.assertEqual(devices[0]["tx_power"], -20)
        self.assertEqual(devices[0]["appearance"], 0)
        self.assertEqual(devices[0]["service_data"], {"0000180a-0000-1000-8000-00805f9b34fb": "74657374"})

    @patch('bluepy.btle.Scanner')
    def test_scan_with_invalid_data(self, mock_scanner):
        """Test scanning with invalid BLE data that would cause conversion errors"""
        ...
        self.assertEqual(len(devices), 1)
        self.assertEqual(devices[0]["mac_address"], "AA:BB:CC:DD:EE:FF")
        self.assertEqual(devices[0]["manufacturer_data"], {"0": "696e76616c69645f64617461"})
        self.assertEqual(devices[0]["service_data"], {"0": "696e76616c69645f736572766963655f64617461"})
